...
- 텍스트·이미지 메시지에 답장하고 인용한 원문으로 이동할 수 있게 한다.
- 최신 위치에서 벗어난 경우 한 번의 동작으로 마지막 메시지로 복귀할 수 있게 한다.
...
상세보기를 열 때 signed URL 또는 로컬 이미지 바이트를 브라우저 Blob으로 미리 준비한다. 저장 버튼을 누르면 `navigator.canShare({files})`가 가능한 모바일 환경에서 네이티브 공유 메뉴를 열고, 그렇지 않으면 Blob URL과 `download` 링크로 파일 다운로드를 시작한다. Web Share는 반드시 사용자 버튼 동작에서 호출하며 취소는 오류로 표시하지 않는다.
...
`messages`에 nullable `reply_to_message_id`와 답장 당시 원문의 작성자명, 본문 일부, 메시지 유형을 담는 인용 스냅샷을 추가한다. 클라이언트는 답장 대상 ID만 제출하고 데이터베이스가 같은 방의 원문인지 확인한 뒤 스냅샷을 확정한다. 따라서 다른 방 메시지를 참조하거나 작성자·본문을 위조할 수 없다.

원문이 이후 삭제되거나 현재 페이지에 로드되지 않아도 인용 스냅샷은 말풍선에 남긴다. 원문 행이 존재하면 인용 영역 선택 시 해당 메시지로 스크롤하고, 현재 목록에 없으면 원문 주변 내역을 조회한 뒤 이동한다. 원문을 더 이상 조회할 수 없으면 인용 내용만 표시하고 이동 동작은 비활성화한다.

답장 작성 중에는 입력창 위에 작성자와 텍스트 일부 또는 `사진`을 표시하고 취소 동작을 제공한다. 전송 실패 후 재시도와 낙관적 메시지에도 동일한 답장 대상과 인용 내용이 유지되어야 한다.

답장 필드를 추가해 메시지 조회 RPC의 반환 형식을 다시 만들 때 기존 `sender_avatar_path`와 이미지 첨부 필드를 함께 유지한다. 최근 내역과 연결 복구 함수가 서로 다른 반환 형식을 갖지 않도록 동일한 열 순서를 사용한다.

### 최신 메시지 이동 버튼

스크롤 위치가 최신 메시지 근처를 벗어나면 입력창 위 우측에 아래쪽 화살표 플로팅 버튼을 표시한다. 버튼을 누르면 마지막 메시지가 보이는 위치로 부드럽게 이동하며, 이미 최신 위치에 도달했거나 메시지가 없으면 숨긴다. 버튼에는 새 메시지 개수나 배지를 표시하지 않는다.

키보드와 입력창 높이 변화를 반영해 버튼이 입력 UI를 가리지 않게 배치한다. 답장 원문으로 이동하거나 이전 내역을 추가 로드한 뒤에도 같은 기준으로 버튼 노출을 다시 계산한다.

### 메시지 복사와 답장 메뉴

텍스트 메시지를 길게 누르면 `전체 복사`, `부분 복사`, `답장`을 하나의 하단 메뉴에 표시한다. 전체 복사는 즉시 클립보드에 저장하고, 부분 복사는 제스처 충돌을 피하기 위해 선택 가능한 텍스트를 별도 창에 표시한다. 이미지 메시지의 길게 누르기는 기존처럼 바로 답장을 선택한다.
...
- [답장 스냅샷 위조 또는 다른 방 참조] → 클라이언트는 대상 ID만 전송하고 DB가 같은 방 원문에서 스냅샷을 생성한다.
- [원문 이동 중 pagination과 스크롤 위치 충돌] → 메시지 ID 기반 위치를 확인하고 필요한 페이지를 로드한 뒤 프레임 완료 후 이동한다.
